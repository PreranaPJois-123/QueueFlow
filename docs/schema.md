...
All tables use UUID primary keys (stored as Postgres `uuid`, generated client-side by
...
|---|---|---|
| id | uuid PK | |
| queue_id | uuid FK → queues.id | indexed |
...
| created_at | timestamptz | indexed — used to order the waiting line FIFO |
